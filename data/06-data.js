// Lesson data extracted from data_test.js
const words = [
  {
    title: "The present simple tence",
    content: [
      {
        use: "Đã xảy ra và đã chấm dứt trong quá khứ",
        practice: [
          {
            sentence_vi: "(Chúng tôi mua ngôi nhà đó năm 2000.)",
            sentence_en: "We bought the house in 2000.",
          },
        ],
      },
      {
        use: "xảy ra liên tiếp trong quá khứ",
        practice: [
          {
            sentence_vi:
              "(Cô ấy bật máy tính, đọc tin nhắn trong Facebook rồi trả lời nó.)",
            sentence_en:
              "She turned on her computer, read the message on Facebook and answered it",
          },
        ],
      },
      {
        use: "hồi ức, kỉ niệm",
        practice: [
          {
            sentence_vi: "(Hồi còn nhỏ, tôi thường đi câu cá vào buổi chiều.)",
            sentence_en:
              "When I was small, I used to go fishing in the afternoon.",
          },
        ],
      },
    ],
    translate: [
      {
        use: "=====================Dịch câu========================",
        practice: [
          {
            sentence_vi: "Mary đã đi mua sắm với tôi ngày hôm qua.",
            sentence_en: "Mary went shopping with me yesterday.",
          },
          {
            sentence_vi: "Viên cảnh sát đã lái xe đi cách đây mười phút.",
            sentence_en: "The policeman drove away ten minutes ago.",
          },
          {
            sentence_vi: "Nhà máy sản xuất bánh quy đã đóng cửa vào năm ngoái.",
            sentence_en: "The biscuit factory closed last year.",
          },
          {
            sentence_vi: "Hiệu trưởng đã đến trường vào năm 1985.",
            sentence_en: "The headmaster came to the school in 1985.",
          },
          {
            sentence_vi: "Bạn đã tiêu nhiều tiền vào tháng trước phải không?",
            sentence_en: "Did you spend a lot of money last month?",
          },
          {
            sentence_vi: "Lần cuối tôi đến Tây Ban Nha là vào năm 1990.",
            sentence_en: "I last went to Spain in 1990.",
          },
          {
            sentence_vi:
              "Cậu bé đã hoàn thành bài tập về nhà vào ngày hôm qua.",
            sentence_en: "The boy finished his homework yesterday.",
          },
          {
            sentence_vi: "Rolf đã mua chiếc áo khoác da vào tuần trước.",
            sentence_en: "Rolf bought the leather jacket last week.",
          },
          {
            sentence_vi:
              "Tôi đã không gặp bất kỳ rắc rối nào với chiếc xe của mình vào tuần trước.",
            sentence_en: "I did not have any trouble with my car last week.",
          },
          {
            sentence_vi: "Cô ấy đã giặt quần áo vào tối qua.",
            sentence_en: "She washed her clothes last night.",
          },
          {
            sentence_vi:
              "Bạn đã làm gì vào cuối tuần trước? Tôi đã ở nhà và ngủ.",
            sentence_en:
              "What did you do last weekend? I stayed at home and slept.",
          },
          {
            sentence_vi: "Khi còn trẻ, cô ấy rất xinh đẹp và hấp dẫn.",
            sentence_en:
              "When she was young, she was beautiful and attractive.",
          },
          {
            sentence_vi: "Bạn đã ở đâu vào tối qua?",
            sentence_en: "Where were you last night?",
          },
          {
            sentence_vi:
              "Người đàn ông bước ra khỏi xe, khóa xe lại và đi vào tòa nhà.",
            sentence_en:
              "The man got out of the car, locked it and came into the building.",
          },
          {
            sentence_vi:
              "Bạn đã tham gia chuyến dã ngoại cùng họ vào tuần trước phải không?",
            sentence_en: "Did you join with them on a picnic last week?",
          },
        ],
      },
      {
        use: "Câu phủ định (-): S + do/does + not + V (nguyên dạng)",
        practice: [
          {
            sentence_vi: "Tôi không thích cà phê.",
            sentence_en: "I don't like coffee.",
          },
        ],
      },
      {
        use: "Câu nghi vấn (?): Do/ does + S + V (nguyên dạng)?",
        practice: [
          {
            sentence_vi: "Bạn có thích cà phê không?",
            sentence_en: "Do you like coffee?",
          },
        ],
      },
    ],
    context: [
      {
        situation:
          "Minh làm việc tại một công ty phần mềm. Anh ấy thường thức dậy lúc 6 giờ 30 mỗi sáng. Sau khi thức dậy, anh ấy uống một cốc nước và tập thể dục khoảng 20 phút. Anh ấy không ăn sáng ở nhà vì thường mua đồ ăn trên đường đến công ty. Công việc của Minh bắt đầu lúc 8 giờ 30. Anh ấy làm việc với một nhóm gồm năm người và thường có cuộc họp vào sáng thứ Hai. Minh thích công việc của mình vì anh ấy được gặp và giải quyết nhiều vấn đề thú vị.",
        practice: [
          {
            sentence_vi: "Minh làm việc tại một công ty phần mềm",
            sentence_en: "Minh works for a software company",
          },
          {
            sentence_vi: "Anh ấy thường thức dậy lúc 6 giờ 30 mỗi sáng",
            sentence_en: "He usually gets up at 6:30 every morning",
          },
          {
            sentence_vi:
              "Sau khi thức dậy, anh ấy uống một cốc nước và tập thể dục khoảng 20 phút",
            sentence_en:
              "After he gets up, he drinks a glass of water and exercises for about twenty minutes",
          },
        ],
      },
    ],
  },
];
