...
function buildLesson(lesson) {
  ...
  const sub = el("div", {
    class: "page-sub lesson-tabs",
    role: "tablist",
    "aria-label": "Lesson sections",
  });
  const usageTab = el("button", {
    id: "usage-tab",
    class: "lesson-tab active",
    type: "button",
    role: "tab",
    "aria-selected": "true",
  ...
  const contentSheet = el("div", {
    class: "sheet lesson-panel",
    id: "usage-panel",
    role: "tabpanel",
    "aria-labelledby": "usage-tab",
  });
  contentSheet.appendChild(
  ...
  if (lesson.translate.length === 0) {
    ...
    lesson.translate.forEach((block) => {
      ...
      if (block.use)
        wrap.appendChild(el("p", { class: "use-title", text: block.use }));
      block.practice.forEach((item) =>
        wrap.appendChild(buildPracticeRow(item)),
      );
  ...
  lesson.context.forEach((ctx) => {
    ...
    ctx.practice.forEach((item) =>
      wrap.appendChild(buildContextPracticeRow(item)),
    );
  ...
  translateTab.addEventListener("click", () =>
    selectTab(translateTab, translateSheet),
  );
  contextTab.addEventListener("click", () =>
    selectTab(contextTab, contextSheet),
  );
}
